    ...
    >
      <div class="app-brand demo">
        <a href="index.php" class="app-brand-link">
          ...
          <span class="app-brand-text demo menu-text fw-bolder ms-2">Sood Avar</span>
        </a>
...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">داشبورد /</span> قبوض
</h4>
...
<a class="btn btn-primary" href="add-bills.php"><i class="bx bx-plus"></i>افزودن قبض</a>
<br /> <br />

  <table id="table" class="table table-striped">
    <thead>
      <tr class="th_style_bills">
        <th>ردیف</th>
        <th>عنوان قبض</th>
        <th>نوع فبض</th>
        <th>حساب بانکی</th>
        <th>تاریخ پرداخت قبض</th>
        <th>مبلغ قابل پرداخت قبض (تومان)</th>
        <th></th>
      </tr>
    </thead>
    
    <?php
      $query = $conn->query("SELECT * FROM `bills`") or die(mysqli_error());
    ...
    ?>
      ...
      <tr class="td_style">
        ...
        <td> <?php echo $fetch['banks']?> </td>
        <td> <?php echo $fetch['date']?> </td>
        <td> <?php echo $fetch['salary']?> </td>
        
        <td> <center>
          <a class="btn btn-label-primary me-2 edit" data-id="<?= $fetch['id'] ?>" data-title="<?= $fetch['title'] ?>" data-type="<?= $fetch['type'] ?>" data-banks="<?= $fetch['banks'] ?>" data-date="<?= $fetch['date'] ?>" data-salary="<?= $fetch['salary'] ?>" data-bs-toggle="modal" data-bs-target="#editbillsModal"><i class="glyphicon glyphicon-edit"> ویرایش </i></a>
...
<!-- edit bills -->
<div class="modal fade" id="editbillsModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-simple modal-add-new-cc">
    <div class="modal-content p-3 p-md-5">
      <div class="modal-body">
        ...
        <form action="dashboards-bills.php" method="post" enctype="multipart/form-data" class="row g-3">

          <input type="text" name="id" class="form-control" id="editID" style="display: none;" >
          
          <div class="col-12">
            <label class="form-label w-100 text_idb margin-title" for="title">عنوان قبض</label>
            <div class="input-group input-group-merge">
              <input type="text" class="form-control" name="title" placeholder="قبض مربوط به چه مکانی است" id="editTitle" required = "required" />
          </div>

          <div class="col-12 col-md-6">
            <label class="form-label text_idb" for="type">نوع قبض</label>
            <select class = "form-control" required = "required" name="type" id="editType">
              <option value="">نوع قبض را انتخاب کنید</option>
              <br />
          ...
          <?php
            $query1 = $conn->query("SELECT numbers FROM `banks`") or die(mysqli_error());
            while($fetch1 = $query1->fetch_array()){
          ?>
          <div class="col-12 col-md-6">
            <label class="form-label text_idb" for="banks">حساب بانکی</label>
            <select class = "form-control" required = "required" name="banks" id="editBanks">
              <option value="">حساب بانکی ای که با آن قبض را پرداخت کرده اید،انتخاب کنید</option>
              <br />
              <option value = "<?php echo $fetch1['numbers'] ?>"><?php echo $fetch1['numbers'] ?></option>
              <br />
            </select>
          </div>
          <?php
          ...
          <div class="col-12 col-md-6">
            <label class="form-label text_idb margin_record" for="date">تاریخ پرداخت قبض</label>
            <input type="text" class="form-control" name="date" placeholder="روز/ماه/سال" id="editDate" required = "required" />
          </div>

          <div class="col-12 col-md-6">
            <label class="form-label text_idb margin_record" for="salary">مبلغ قابل پرداخت قبض</label>
...
<!-- edit script -->
  <script>
    ...
    let editBanks = document.querySelector('#editBanks');
    ...
    editBtns.forEach(editBtn => {
      editBtn.addEventListener('click', () => {
        ...
        let typeID;
        if (editBtn.dataset.type == "آب") {
          typeID = 1;
        } else if (editBtn.dataset.type == "برق"){
          typeID = 2
        } else if (editBtn.dataset.type == "گاز"){
          typeID = 3
        } else if (editBtn.dataset.type == "تلفن"){
          typeId = 4
        } else {
          typeID = 5
        }
        ...
        let banksID;
        if (editBtn.dataset.banks == "ملت") {
          banksID = 1;
        } else if (editBtn.dataset.banks == "ملی"){
          banksID = 2
        } else if (editBtn.dataset.banks == "سپه"){
          banksID = 3
        } else if (editBtn.dataset.banks == "رسالت"){
          banksId = 4
        } else {
          banksID = 5
        }
        editBanks.selectedIndex = banksID
      })
    })
  </script>
...
<!-- delete script -->
  <script type = "text/javascript">
    function confirmationDelete(anchor){
      var conf = confirm(" آیا مطمئن به حذف این قبض هستید؟ ");
      if(conf){
